...
public class PointDeTrace : Point
    {
        ...
        public PointDeTrace(double uneLatitude, double uneLongitude, double uneAltitude, DateTime uneDateHeure,
        int unRythmeCardio, long unTempsCumule, double uneDistanceCumulee, double uneVitesse)
            // appelle le constructeur de la classe mère
            : base(uneLatitude, uneLongitude, uneAltitude)
        {
            // initialise les nouveaux champs
        ...
        {
            // initialise les nouveaux champs
            this._latitude = unPointExistant.getLatitude();
            this._longitude = unPointExistant.getLongitude();
            this._altitude = unPointExistant.getAltitude();
            this._dateHeure = unPointExistant.getDateHeure();
            this._rythmeCardio = unPointExistant.getRythmeCardio();
            this._tempsCumule = unPointExistant.getTempsCumule();
            this._distanceCumulee = unPointExistant.getDistanceCumulee();
            this._vitesse = unPointExistant.getVitesse();
        }
        ...
        {
            long heures = (_tempsCumule / 3600);
            long reste = _tempsCumule - heures * 3600;
